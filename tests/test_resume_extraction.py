...
def test_extract_resumes_and_save_to_csv():
    print("Starting test_extract_resumes_and_save_to_csv test...")
    ...
    if extracted_data:
        ...
        csv_path = os.path.join(project_root, "extracted_resumes_full.csv")
        ...
        raw_resume_path = os.path.join(project_root, "data", "raw_resume.csv")
        assert os.path.exists(raw_resume_path), f"Raw Resume CSV file {raw_resume_path} was not created"
        raw_df = pd.read_csv(raw_resume_path)
        assert len(raw_df) >= len(extracted_data), f"Raw Resume CSV file contains {len(raw_df)} rows, expected at least {len(extracted_data)}"
        print("Raw Resume CSV file validation passed")
    else:
        print("No content was extracted")
        assert False, "No content was extracted from any file"

if __name__ == "__main__":
    test_extract_resumes_and_save_to_csv() 